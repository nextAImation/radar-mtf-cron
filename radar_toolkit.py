...
if __name__ == "__main__":
    ...
    price = 100 + np.cumsum(np.random.normal(0.15, 0.9, size=n))
    high = price + np.random.uniform(0.2, 1.2, size=n)
    low = price - np.random.uniform(0.2, 1.2, size=n)
    open_ = price + np.random.normal(0, 0.4, size=n)
    close = price + np.random.normal(0, 0.4, size=n)
    ...
    assert {"ema20","ema50","ema200","atr14","rsi14","adx14","di_plus14","di_minus14",
            "vol_ma20","vol_ma60","vol_ratio","tr","vol_z60","tr_z60",
            "sw_high20","sw_low20","fib_retrace","stacked","near_ema","candle_ok",
            "dyn_res","dist_res_atr","avwap"}.issubset(ind.columns)
    print("TOOLKIT_OK")
